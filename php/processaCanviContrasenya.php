...
if ($cValid) {
    ...
    if ($passconf == $passwordnew) {
        ...
        return header("Location: $url?parametre=exitopass");
    } else {
        $host = $_SERVER['HTTP_HOST'];
        // $ruta = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
        $html = 'usuariRegistrat.php';
        $url = "http://$host/php/$html";
        return header("Location: $url?parametre=modpass&error=confc");
    }
} else {
    $host = $_SERVER['HTTP_HOST'];
    // $ruta = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
    $html = 'usuariRegistrat.php';
    $url = "http://$host/php/$html";
    return header("Location: $url?parametre=modpass&error=cactual");
}
